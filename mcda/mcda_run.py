...
def main(input_config: dict):
    ...
    if is_variability == "no":
        logger.info("ProMCDA will not consider the score's variability due to the different norm/agg functions")
        f_norm = config.variability['normalization']
        f_agg = config.variability['aggregation']
    else:
        logger.info("ProMCDA will consider the score's variability due to the different norm/agg functions")
    if is_sensitivity == "no":
        logger.info("ProMCDA will run without uncertainty on the indicators")
        is_uncertainty = 0
        ...
        logger.info("Number of Monte Carlo runs: {}".format(config.monte_carlo_sampling["monte_carlo_runs"]))
    ...
    if (config.sensitivity["on_single_weights"] == "no" and config.sensitivity["on_all_weights"] == "no"):
        ...
        num_runs = config.monte_carlo_sampling["monte_carlo_runs"]
        is_random_w_iterative = config.sensitivity["on_single_weights"]
        is_random_w_all = config.sensitivity["on_all_weights"]
        if is_random_w_iterative == "no" and is_random_w_all == "yes":
        ...
        elif is_random_w_iterative == "yes" and is_random_w_all == "no":
            i=0
    ...
    if is_uncertainty == 0:
        if (config.monte_carlo_sampling["monte_carlo_runs"] > 0):
            ...
            logger.info("Start MCDA without uncertainty of the indicators")
            t = time.time()
            scores = pd.DataFrame()
            all_weights_means = pd.DataFrame()
            mcda_no_uncert = MCDAWithoutUncertainty(config, input_matrix_no_alternatives)
    ...
    # -------------------------
        ...
        if (mc_runs > 0):
            if (mc_runs < 1000):
                ...
                time.sleep(5)
            logger.info("Start MCDA with uncertainty on the indicators")
            t = time.time()
