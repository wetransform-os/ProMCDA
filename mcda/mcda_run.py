...
def main(input_config: dict):
    ...
    is_variability = config.variability['variability_on']
    is_sensitivity = config.sensitivity['robustness_on']
    mc_runs = config.monte_carlo_sampling["monte_carlo_runs"]
    if is_variability == "no":
        f_norm = config.variability['normalization']
        f_agg = config.variability['aggregation']
    ...
    if is_sensitivity == "no":
        logger.info("ProMCDA will run without robustness on the indicators or weights")
        logger.info("Read input matrix without robustness at {}".format(config.input_matrix_path))
    else: # robustness yes
        if (config.sensitivity["on_single_weights"] == "no"
                and config.sensitivity["on_all_weights"] == "no"
                and config.sensitivity["on_indicators"] == "no"):
            logger.error('Error Message', stack_info=True)
            raise ValueError('Sensitivity analysis is requested but where is not defined: weights or indicators? Please clarify.')
        if (config.sensitivity["on_single_weights"] == "yes"
                and config.sensitivity["on_all_weights"] == "yes"
                and config.sensitivity["on_indicators"] == "no"):
            logger.error('Error Message', stack_info=True)
            raise ValueError('Sensitivity analysis is requested on the weights: but on all or one at time? Please clarify.')
        if ((config.sensitivity["on_single_weights"] == "yes"
                and config.sensitivity["on_all_weights"] == "yes"
                and config.sensitivity["on_indicators"] == "yes") or
            (config.sensitivity["on_single_weights"] == "yes"
               and config.sensitivity["on_all_weights"] == "no"
               and config.sensitivity["on_indicators"] == "yes") or
            (config.sensitivity["on_single_weights"] == "no"
               and config.sensitivity["on_all_weights"] == "yes"
               and config.sensitivity["on_indicators"] == "yes")):
            logger.error('Error Message', stack_info=True)
            raise ValueError('Sensitivity analysis is requested: but on weights or indicators? Please clarify.')
        if ((config.sensitivity["on_single_weights"] == "yes"
                 and config.sensitivity["on_all_weights"] == "no"
                 and config.sensitivity["on_indicators"] == "no") or
                (config.sensitivity["on_single_weights"] == "no"
                 and config.sensitivity["on_all_weights"] == "yes"
                 and config.sensitivity["on_indicators"] == "no")):
            logger.info("ProMCDA will consider robustness on the weights")
            is_robustness_weights = 1
            logger.info("Number of Monte Carlo runs: {}".format(mc_runs))
        elif (config.sensitivity["on_single_weights"] == "no"
              and config.sensitivity["on_all_weights"] == "no"
              and config.sensitivity["on_indicators"] == "yes"):
            logger.info("ProMCDA will consider robustness on the indicators")
    ...
    if (is_robustness_indicators==1 & (len(input_matrix.columns) - 1) % 2 != 0): # Alternatives column is still in, therefore -1
    ...
    if (is_robustness_weights == 0):
        fixed_weights = config.sensitivity["given_weights"]
        ...
        if config.sensitivity["on_single_weights"] == "no" and config.sensitivity["on_all_weights"] == "yes":
        ...
        elif config.sensitivity["on_single_weights"] == "yes" and config.sensitivity["on_all_weights"] == "no":
            i=0
    ...
    if is_robustness_indicators == 0:
        ...
        if (config.sensitivity["on_single_weights"] == "no" and config.sensitivity["on_all_weights"] == "no"): # FIXED WEIGHTS
            if is_variability == "yes":
        ...
        elif config.sensitivity["on_single_weights"]  == "yes": # ONE RANDOMLY SAMPLED WEIGHT A TIME (MCDA runs (num_samples * num_indicators) times)
