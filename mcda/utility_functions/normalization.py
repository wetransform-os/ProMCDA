...
class Normalization(object):
    ...
    def target(self, feature_range: tuple) -> pd.DataFrame():
        ...
        if self._input_matrix.values.any() == 0:
            epsilon = 0.1
        elif self._input_matrix.values.any() > 0:
            epsilon = 0

        if (feature_range == (0, 1)):
            indicators_scaled_target_plus = indicators_plus / (indicators_plus.max(axis=0) + epsilon) # for + polarity
            indicators_scaled_target_minus = 1 - indicators_minus / (indicators_minus.max(axis=0) + epsilon)  # for - polarity
        else:
            indicators_scaled_target_plus = indicators_plus / (indicators_plus.max(axis=0) + epsilon) * (
                        1 - 0.1) + 0.1  # for + polarity
            indicators_scaled_target_minus = (1 - indicators_minus / (indicators_minus.max(axis=0) + epsilon)) * (
                        1 - 0.1) + 0.1  # for - polarity
